...
from thermalmodels_first import FirstOrderModel, FirstOrderThermostat
...
class DirectSetpointEnv(gym.Env):
    ...
    def _getObservation(self, temp=None):
        ...
        return np.array([temp,self.price, self.price_2, self.price_3, self.price_4, self.secondsON, self.cost,self.comfort, self.time,self.status,self.target,self.previous_temp1,self.previous_temp2,self.previous_temp3,self.previous_temp4,self.task1,self.task2,self.task3,self.task4,self.task5]) 