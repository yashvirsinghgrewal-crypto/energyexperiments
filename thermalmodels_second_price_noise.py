...
class SecondOrderModel(object):
    ...
    """
    def __init__(self, pStepSeconds=10):
        
        # Get the building constants
        self.ConstantsGLM = ConstantsGLM(1)
    ...
    def advance( self,Tout, TMassAir, Qi, Qh, noise): 
        ...
        TMassAir['AIR'] = e1 + e2 + Teq + noise # Noise is the std deviation
...
class SecondOrderThermostat(object):
  ...
  #      this.mode = initMode;
    ...
    def advance(self, tOut, deltaSecs, next_price,time,noise=0):  # Next price
        ...
        stepsize = 10
        alpha = 1.0  # Weight on cost level
        beta = 1
        if (next_price >= 80):
            beta = 1  # was 0.1
            alpha = alpha * 2.5
