...
class DirectSetpointEnv(gym.Env):
   ...
   # OUT = np.random.normal(5, 4, 1)
    ...
    # Num steps = 1 day.
    STEPS = (24 * 60 * 60) / DELTA
    STEPS += 40  # For exploration phase of the exploration policy only, comment this when testing Rl2 vanilla
    ...
    #        self.OUT= np.random.normal(self.OUT, 1, 1)[0]
       ...
       # Ensure valid action chosen.
        ...
        self.previous_env_reward = self.reward
        self.reward = reward
        
        return output, reward, done, {'SecondsON': self.secondsON, 'Price': previous,'Action':self.action_previous,'Cost' : self.cost,'Comfort': self.comfort }
    ...
    def _getObservation(self, temp = None):
        ...
        return np.array([self.model.tIn['AIR'],self.price, self.price_2, self.price_3, self.price_4, self.secondsON, self.cost,self.comfort, self.time,self.status,self.target, self.action_previous, self.previous_env_reward]) 
