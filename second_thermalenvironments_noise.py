...
from thermalmodels_second_noise import SecondOrderModel, SecondOrderThermostat
   ...
   # OUT = np.random.normal(5, 4, 1)
    ...
    def __init__(self, model,prices_array):
     ...
     #   self.policy_3 = policy_3 # The policy another one
        ...
        low = np.array(
            [np.float32(np.NINF)]*20) #+2 for task encoding
        high = np.array(
    ...
    #        self.OUT= np.random.normal(self.OUT, 1, 1)[0]
        ...
        if self.time in (100,110) or self.time in (200,215):
            self.noise = np.random.uniform(low=-0.5, high=0.5, size=(1,))[0]
       ...
       # Ensure valid action chosen.
        ...
        meantemp, self.secondsON, reward, _, _,self.cost,self.comfort = self.model.advance(self.OUT, self.DELTA, self.price,self.status,self.noise)
        self.time = self.time + 1
    ...
    def reset(self):
        ...
        random_temp = np.random.uniform(low=20, high=26, size=(1,))[0]
