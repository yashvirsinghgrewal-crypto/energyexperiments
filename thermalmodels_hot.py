...
class FirstOrderThermostat(object):
    ...
    def advance(self, tOut, deltaSecs, next_price,status):  # Next price
        ...
        stepsize = 10
        alpha = 1.5 # Weight on cost level
        beta = 1
        if (next_price >= 60):
            beta = 1  # was 0.1
            alpha = alpha * 2.5
        if (self.tIn < 17.5 or self.tIn > 25):
            ...
            aplha = alpha* 0.5
        price_kw = price / 1000
        price_kw = max(price_kw,0) # Not taking -ve prices into account
     ...
     #   total_cost = (0.00007) * sumSecsOn 
        cost = (total_cost - min_cost) / (max_cost - min_cost)
        
        comfort = (sumComfort - min_comfort) / (max_comfort - min_comfort)
        reward = (-alpha * cost) + status*beta * comfort
    
      #  if status == 0: # Must stay OFF when status = OFF
      #      reward = (-alpha * cost)      
            
      #  elif status == 1:     # Must stay ON when status = ON
      #      reward = (-alpha * cost) + beta * comfort
    
    
    
        #Reward is only based on comforta
      #  reward = comfort
        
        #Reward is also based on a fixed schedule from step = 60-66, step = 200-206
        #Normalis
    #    if time_step in range(60,66) or time_step in range(200,206):
    #        reward = (-2.2 * cost ) + 1 * comfort
